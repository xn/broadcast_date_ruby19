class Date
  zzz = Module.new do
    ...
    # [broadcast_year, week_of_year, day_of_week]
    def jd_to_broadcast(jd, sg=GREGORIAN)
      a = jd_to_civil(jd - 3, sg)[0]
      y = if jd >= broadcast_to_jd(a + 1, 1, 1, sg) then a + 1 else a end
      w = 1 + ((jd - broadcast_to_jd(y, 1, 1, sg)) / 7).floor
      d = (jd + 1) % 7
      d = 7 if d == 0
      return y, w, d
    end

    def bweeknum_to_jd(y, w, d, f=0, sg=GREGORIAN) # :nodoc:
      a = find_fdoy(y, sg) + 6
      (a - ((a - f) + 1) % 7 - 7) + 7 * w + d
    end

    def jd_to_bweeknum(jd, f=0, sg=GREGORIAN) # :nodoc:
    ...
    def jd_to_bwday(jd) (jd) % 7 end # :nodoc:
    ...
    #
    # +sg+ specifies the Day of Calendar Reform.
    def _valid_broadcast? (y, w, d, sg=GREGORIAN) # :nodoc:
      if d < 0
        d += 8
      end
      if w < 0
        ny, nw, nd = jd_to_broadcast(broadcast_to_jd(y + 1, 1, 1, sg) + w * 7, sg)
        return unless ny == y
        w = nw
      end
      jd = broadcast_to_jd(y, w, d, sg)
      return unless [y, w, d] == jd_to_broadcast(jd, sg)
      jd
    end

    def _valid_bweeknum? (y, w, d, f, sg=GREGORIAN) # :nodoc:
      if d < 0
        d += 7
      end
      if w < 0
        ny, nw, nd, nf = jd_to_bweeknum(weeknum_to_jd(y + 1, 1, f, f, sg) + w * 7, f, sg)
        return unless ny == y
        w = nw
      end
      jd = bweeknum_to_jd(y, w, d, f, sg)
      return unless [y, w, d] == jd_to_bweeknum(jd, f, sg)
      jd
    end
  end
  extend  zzz
  ...
  def self.valid_broadcast? (y, w, d, sg=ITALY)
    !!_valid_broadcast?(y, w, d, sg)
  end

  def self.valid_bweeknum? (y, w, d, f, sg=ITALY) # :nodoc:
    !!_valid_bweeknum?(y, w, d, f, sg)
  end
  ...
  def self.broadcast(y=-4712, w=1, d=1, sg=ITALY)
    unless jd = _valid_broadcast?(y, w, d, sg)
      raise ArgumentError, 'invalid date'
    end
    new!(jd_to_ajd(jd, 0, 0), 0, sg)
  end

  def self.bweeknum(y=-4712, w=0, d=1, f=0, sg=ITALY)
    unless jd = _valid_bweeknum?(y, w, d, f, sg)
      raise ArgumentError, 'invalid date'
    end
    new!(jd_to_ajd(jd, 0, 0), 0, sg)
  end
  ...
  def self.complete_frags(elem) # :nodoc:
    ...
    g = [[:time, [:hour, :min, :sec]],
      ...
      [:broadcast, [:cwyear, :cweek, :cwday, :hour, :min, :sec]],
...
class DateTime < Date
  ...
  # broadcast Date and hour +h+, minute +min+, second +s+.
  #
  ...
  def self.broadcast(y=-4712, w=1, d=1, h=0, min=0, s=0, of=0, sg=ITALY)
    unless (jd = _valid_broadcast?(y, w, d, sg)) &&
  ...
  def self.bweeknum(y=-4712, w=0, d=1, f=0, h=0, min=0, s=0, of=0, sg=ITALY) # :nodoc:
    unless (jd = _valid_bweeknum?(y, w, d, f, sg)) &&
        (fr = _valid_time?(h, min, s))
      raise ArgumentError, 'invalid date'
    end
